...
class Program
{
    ...
    static void Main()
    {
        ...
        for (int i = 1; i <= n; i++)
            graph[i].Sort((a, b) => order[a].CompareTo(order[b]));
    ...
    static bool BFS(int[] input)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            // 자식 노드 수만큼 input 배열에서 검사
            for (int i = 0; i < childCount; i++)
            {
                if (idx >= n || visited[input[idx]] || !graph[current].Contains(input[idx]))
                    return false;

                visited[input[idx]] = true;
                queue.Enqueue(input[idx]);
                idx++;
            }
        }
